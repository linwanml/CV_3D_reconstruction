...
function [] = reconstruction_demo()
    ...
    for iBegin = 1:n-(numFrames - 1)
        ...
        colInds = find(block(1,:)~=0 & block(2,:)~=0 & block(3,:)~=0);
        ...
        for f = 1:numFrames
            for p = 1:numPoints
                c = C{iBegin + f - 1};
                X(2 * f - 1, p) = c(1,block(f,p));
                X(2 * f, p)     = c(2,block(f,p));
            end
        end
...
%    surfaceRenderBack(mergedCloud, M1, MeanFrame1, im2double(imread(strcat(directory, Files(9).name))), T9);
end
